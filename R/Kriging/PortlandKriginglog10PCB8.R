...
# Kriging
krige_result <- krige(log_PCB8 ~ 1, pcbi_unique, grid_masked, 
                      model = manual_vario,
                      nmax = 10)
...
writeRaster(krige_pred_orig, "Output/GeoData/PCB8_Kriging_Stack.tif",
...
st_write(contours_sf, "Output/GeoData/PCB8_Kriging_Contours.gpkg",
         delete_dsn = TRUE)
...
st_write(pcb_sf_wgs84, "Output/GeoData/PCB8_Sampling_Points.gpkg", delete_dsn = TRUE)

# Plot
ggplot() +
  geom_raster(data = krige_df, aes(x = x, y = y, fill = 10^(pcb_pred))) +
  scale_fill_viridis_c(option = "C", na.value = NA, name = "PCB 8 (pg/m³)") +
  geom_point(data = pcb_df, aes(x = Longitude, y = Latitude, size = PCB8),
