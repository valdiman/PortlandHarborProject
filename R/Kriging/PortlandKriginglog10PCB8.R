# Codes to perform a Kriging analysis for PCB 8
...
# Select PCB
pcbi <- pcb_air %>%
  select(Latitude, Longitude, PCB8) %>%
  filter(!is.na(PCB8))
...
coords_df$PCB8 <- pcbi_utm$PCB8

unique_coords <- coords_df %>%
  group_by(coords.x1, coords.x2) %>%
  summarise(PCB8_mean = mean(PCB8, na.rm = TRUE),
            n_samples = n(),
            .groups = "drop")

pcbi_unique <- unique_coords
coordinates(pcbi_unique) <- ~coords.x1 + coords.x2
pcbi_unique$PCB8 <- pcbi_unique$PCB8_mean
proj4string(pcbi_unique) <- proj4string(pcbi_utm)

pcbi_unique$log_PCB8 <- log10(pcbi_unique$PCB8) # use log10 scale

empirical_vario <- variogram(log_PCB8 ~ 1, pcbi_unique, 
                             cutoff = 10000,
                             width = 1000)
...
krige_result <- krige(log_PCB8 ~ 1, pcbi_unique, grid_tight, 
                      model = manual_vario,
                      nmax = 10)
...
pcb8_df  <- as.data.frame(pcbi_unique)

ggplot() +
  geom_raster(data = krige_df, aes(x = x, y = y, fill = pred_original)) +
  scale_fill_viridis_c(option = "C", na.value = NA, name = "PCB 4 (pg/m³)") +
  geom_point(data = pcb8_df, aes(x = coords.x1, y = coords.x2, size = PCB8_mean),
             color = "blue", alpha = 0.7) +
  theme_minimal() +
  coord_equal() +
  labs(title = "Kriging Prediction of PCB 8 Concentration")
...
moran_test <- Moran.I(pcbi_unique$PCB8, dist_inv)
print("Moran's I test for spatial autocorrelation:")
print(moran_test)

# Leave-one-out cross-validation
krige_cv <- krige.cv(log_PCB8 ~ 1, pcbi_unique, manual_vario)
...
writeRaster(krige_raster_qgis, "Output/GeoData/kriging_pcb8.tif", format = "GTiff", overwrite = TRUE)
pcb8_sf <- st_as_sf(pcb8_df, coords = c("coords.x1", "coords.x2"), crs = st_crs(pcbi_unique))
pcb8_sf_wgs84 <- st_transform(pcb8_sf, 4326)
st_write(pcb8_sf_wgs84, "Output/GeoData/pcb8_sampling_points.gpkg", delete_dsn = TRUE)
