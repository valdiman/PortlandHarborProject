# Codes to perform a Kriging analysis for PCB 31
...
# Select PCB
pcbi <- pcb_air %>%
  select(Latitude, Longitude, PCB31) %>%
  filter(!is.na(PCB31))
...
coords_df$PCB31 <- pcbi_utm$PCB31

unique_coords <- coords_df %>%
  group_by(coords.x1, coords.x2) %>%
  summarise(PCB31_mean = mean(PCB31, na.rm = TRUE),
            n_samples = n(),
            .groups = "drop")

pcbi_unique <- unique_coords
coordinates(pcbi_unique) <- ~coords.x1 + coords.x2
pcbi_unique$PCB31 <- pcbi_unique$PCB31_mean
proj4string(pcbi_unique) <- proj4string(pcbi_utm)

pcbi_unique$log_PCB31 <- log10(pcbi_unique$PCB31) # use log10 scale

# Variogram and model
empirical_vario <- variogram(log_PCB31 ~ 1, pcbi_unique, 
                             cutoff = 10000,
                             width = 1000)
...
# Kriging
krige_result <- krige(log_PCB31 ~ 1, pcbi_unique, grid_masked, 
                      model = manual_vario,
                      nmax = 10)
...
writeRaster(krige_pred_orig, "Output/GeoData/PCB31_Kriging_Stack.tif",
...
st_write(contours_sf, "Output/GeoData/PCB31_Kriging_Contours.gpkg",
         delete_dsn = TRUE)
...
st_write(pcb_sf_wgs84, "Output/GeoData/PCB31_Sampling_Points.gpkg", delete_dsn = TRUE)

# Plot
ggplot() +
  geom_raster(data = krige_df, aes(x = x, y = y, fill = 10^(pcb_pred))) +
  scale_fill_viridis_c(option = "C", na.value = NA, name = "PCB 31 (pg/m³)") +
  geom_point(data = pcb_df, aes(x = Longitude, y = Latitude, size = PCB31),
...
moran_test <- Moran.I(pcbi_unique$PCB31, dist_inv)
