# Codes to perform a Kriging analysis for PCBs 20+28
...
# Select PCB
pcbi <- pcb_air %>%
  select(Latitude, Longitude, 'PCB20+28') %>%
  filter(!is.na('PCB20+28'))
...
coords_df$PCB20_28 <- pcbi_utm$PCB20.28

unique_coords <- coords_df %>%
  group_by(coords.x1, coords.x2) %>%
  summarise(PCB20_28_mean = mean(PCB20_28, na.rm = TRUE),
            n_samples = n(),
            .groups = "drop")

pcbi_unique <- unique_coords
coordinates(pcbi_unique) <- ~coords.x1 + coords.x2
pcbi_unique$PCB20_28 <- pcbi_unique$PCB20_28_mean
proj4string(pcbi_unique) <- proj4string(pcbi_utm)

pcbi_unique$log_PCB20_28 <- log10(pcbi_unique$PCB20_28) # use log10 scale

# Variogram and model
empirical_vario <- variogram(log_PCB20_28 ~ 1, pcbi_unique, 
                             cutoff = 10000,
                             width = 1000)
...
# Kriging
krige_result <- krige(log_PCB20_28 ~ 1, pcbi_unique, grid_masked, 
                      model = manual_vario,
                      nmax = 10)
...
writeRaster(krige_pred_orig, "Output/GeoData/PCB20.28_Kriging_Stack.tif",
...
st_write(contours_sf, "Output/GeoData/PCB20.28_Kriging_Contours.gpkg",
         delete_dsn = TRUE)
...
st_write(pcb_sf_wgs84, "Output/GeoData/PCB20.28_Sampling_Points.gpkg", delete_dsn = TRUE)

# Plot
ggplot() +
  geom_raster(data = krige_df, aes(x = x, y = y, fill = 10^(pcb_pred))) +
  scale_fill_viridis_c(option = "C", na.value = NA, name = "PCB 20+28 (pg/m³)") +
  geom_point(data = pcb_df, aes(x = Longitude, y = Latitude, size = PCB20_28),
             color = "blue", alpha = 0.7) +
  theme_minimal() +
  coord_equal() +
  labs(title = "Kriging Prediction of PCBs 20+28 Concentration")
...
moran_test <- Moran.I(pcbi_unique$PCB20_28, dist_inv)
print("Moran's I test for spatial autocorrelation:")
print(moran_test)

# Leave-one-out cross-validation
krige_cv <- krige.cv(log_PCB20_28 ~ 1, pcbi_unique, manual_vario)
