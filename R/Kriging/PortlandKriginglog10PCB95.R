# Codes to perform a Kriging analysis for PCB 95
...
# Select PCB
pcbi <- pcb_air %>%
  select(Latitude, Longitude, PCB95) %>%
  filter(!is.na(PCB95))
...
coords_df$PCB95 <- pcbi_utm$PCB95

unique_coords <- coords_df %>%
  group_by(coords.x1, coords.x2) %>%
  summarise(PCB95_mean = mean(PCB95, na.rm = TRUE),
            n_samples = n(),
            .groups = "drop")

pcbi_unique <- unique_coords
coordinates(pcbi_unique) <- ~coords.x1 + coords.x2
pcbi_unique$PCB95 <- pcbi_unique$PCB95_mean
proj4string(pcbi_unique) <- proj4string(pcbi_utm)

pcbi_unique$log_PCB95 <- log10(pcbi_unique$PCB95) # use log10 scale

# Variogram and model
empirical_vario <- variogram(log_PCB95 ~ 1, pcbi_unique, 
                             cutoff = 10000,
                             width = 1000)

manual_vario <- fit.variogram(empirical_vario, 
                              model = vgm(psill = 0.15,
                                          model = "Sph",
                                          range = 5000,
                                          nugget = 0.077))

# Data do not show any detectable spatial correlation at the distances sampled
...
# Kriging
krige_result <- krige(log_PCB95 ~ 1, pcbi_unique, grid_masked, 
                      model = manual_vario,
                      nmax = 10)
...
writeRaster(krige_pred_orig, "Output/GeoData/PCB95_Kriging_Stack.tif",
...
st_write(contours_sf, "Output/GeoData/PCB95_Kriging_Contours.gpkg",
         delete_dsn = TRUE)
...
st_write(pcb_sf_wgs84, "Output/GeoData/PCB95_Sampling_Points.gpkg", delete_dsn = TRUE)

# Plot
ggplot() +
  geom_raster(data = krige_df, aes(x = x, y = y, fill = 10^(pcb_pred))) +
  scale_fill_viridis_c(option = "C", na.value = NA, name = "PCB 95 (pg/m³)") +
  geom_point(data = pcb_df, aes(x = Longitude, y = Latitude, size = PCB95),
             color = "blue", alpha = 0.7) +
  theme_minimal() +
  coord_equal() +
  labs(title = "Kriging Prediction of PCB 95 Concentration")
...
moran_test <- Moran.I(pcbi_unique$PCB95, dist_inv)
print("Moran's I test for spatial autocorrelation:")
print(moran_test)

# Leave-one-out cross-validation
krige_cv <- krige.cv(log_PCB95 ~ 1, pcbi_unique, manual_vario)
...
cat("Moran's I p-value:", round(moran_test$p.value, 8), "\n")
cat("Variogram R²:", round(vario_r2, 3), "\n\n")
...
cat("RMSE:", round(cv_metrics_original$RMSE, 2), "pg/m³\n")
cat("MAE:", round(cv_metrics_original$MAE, 2), "pg/m³\n")
