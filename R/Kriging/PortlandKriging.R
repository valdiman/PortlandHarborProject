...
empirical_vario <- variogram(PCB11 ~ 1, pcbi_unique, 
                             cutoff = 10000,
                             width = 1000)
...
krige_result <- krige(PCB11 ~ 1, pcbi_unique, grid_tight, 
                      model = manual_vario,
                      nmax = 10)

krige_raster <- raster(krige_result["var1.pred"])

plot(krige_raster, main = "PCB11 Concentration Prediction")
points(pcbi_unique, pch = 16, col = "red")

mapview(krige_raster, layer.name = "PCB11 Prediction") + 
  mapview(pcbi_unique, zcol = "PCB11")
...
krige_cv <- krige.cv(PCB11 ~ 1, pcbi_unique, manual_vario)
